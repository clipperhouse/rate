...
func TestLimiter_Wait_SingleBucket(t *testing.T) {
	...
	t.Run("Timed_Cancellation", func(t *testing.T) {
		...
		cancellationTime := executionTime.Add(limit.durationPerToken / 2)
		// Simulate cancellation by closing the done channel after the deadline
		go func(startTime time.Time) {
			time.Sleep(cancellationTime.Sub(startTime))
			close(doneCh)
		}(executionTime)
...
func TestLimiter_WaitN_SingleBucket(t *testing.T) {
	...
	t.Run("Timed_Cancellation", func(t *testing.T) {
		...
		cancellationTime := executionTime.Add(limit.durationPerToken)
		// Simulate cancellation by closing the done channel after the deadline
		go func(startTime time.Time) {
			time.Sleep(cancellationTime.Sub(startTime))
			close(doneCh)
...
func TestLimiter_Wait_MultipleBuckets(t *testing.T) {
	...
	for bucketID := range buckets {
		...
		allow := limiter.waitWithCancellation(bucketID, executionTime, done)
...
func TestLimiter_Wait_MultipleBuckets_Concurrent(t *testing.T) {
	...
	t.Run("Limited_Time", func(t *testing.T) {
		...
		// Done channel that closes when cancellation occurs to simulate real context behavior
		doneCh := make(chan struct{})
		done := func() <-chan struct{} {
			return doneCh
		}
		...
		// Simulate cancellation by closing the done channel after the deadline
		cancellationTime := executionTime.Add(limit.durationPerToken * 3)
		go func(startTime time.Time) {
			time.Sleep(cancellationTime.Sub(startTime))
			close(doneCh)
		}(executionTime)
		...
		for i := range concurrency {
			...
			go func(i int64) {
				defer wg.Done()
				bucketID := i % buckets
				results[i] = limiter.waitWithCancellation(bucketID, executionTime, done)
			}(i)
		}

		wg.Wait()

		// We waited for the cancellation period
		...
		// We expect fewer successes than total goroutines due to the cancellation
	...
	t.Run("Early_Deadline", func(t *testing.T) {
		...
		// Deadline that expires too soon
		cancellationTime := currentTime.Add(limit.durationPerToken / 2)
		// Simulate cancellation by closing the done channel after the deadline
		go func(startTime time.Time) {
			time.Sleep(cancellationTime.Sub(startTime))
			close(doneCh)
		...
		for i := range concurrency {
			...
			go func(i int64) {
				defer wg.Done()
				results[i] = limiter.waitWithCancellation(i, currentTime, done)
			}(i)
		}

		wg.Wait()

		// All should fail because cancellation occurs before tokens are available
	...
	t.Run("Immediate_Cancellation", func(t *testing.T) {
		...
		for i := range concurrency {
			...
			go func(i int64) {
				defer wg.Done()
				results[i] = limiter.waitWithCancellation(i, executionTime, done)
			}(i)
		}
...
func TestLimiter_WaitN_ConsumesCorrectTokens(t *testing.T) {
	...
	t.Run("One_Token", func(t *testing.T) {
		...
		allowed := limiter.waitWithCancellation("test-wait-1", executionTime, done)
	...
	t.Run("N_Tokens", func(t *testing.T) {
		...
		allowed := limiter.waitNWithCancellation("test-waitn-3", executionTime, tokensToWait, done)
	...
	t.Run("MultipleLimits_N_Tokens", func(t *testing.T) {
		...
		allowed := limiter.waitNWithCancellation("test-multi-waitn", executionTime, tokensToWait, done)
	...
	t.Run("Zero_Tokens", func(t *testing.T) {
		...
		// Done channel that closes immediately to simulate immediate cancellation
		doneCh := make(chan struct{})
		...
		allowed := limiter.waitNWithCancellation("test-fail-waitn", executionTime, 1, done)
		require.False(t, allowed, "waitN should fail when cancellation occurs before tokens available")
	...
	t.Run("High_Token_Count", func(t *testing.T) {
		...
		// WaitN should succeed and consume exactly tokensToWait tokens
		allowed := bigLimiter.waitNWithCancellation("test-big-waitn", executionTime, tokensToWait, done)
	...
	t.Run("Concurrent_Consumption", func(t *testing.T) {
		...
		for i := range numGoroutines {
			...
			go func(i int) {
				defer wg.Done()
				results[i] = concurrentLimiter.waitNWithCancellation("test-concurrent-waitn", executionTime, tokensPerWait, done)
			}(i)
		}
...
func TestLimiter_Wait_FIFO_Ordering_SingleBucket(t *testing.T) {
	...
	for i := range concurrency {
		go func(id int) {
			...
			// Wait for a token
			if limiter.waitWithCancellation("key", executionTime, done) {
...
func TestLimiter_Wait_FIFO_Ordering_MultipleBuckets_Flaky(t *testing.T) {
	...
	for i := range concurrency {
		go func(id int) {
			...
			// Wait for a token
			if limiter.waitWithCancellation(bucketID, executionTime, done) {
...
func TestLimiter_WaitersCleanup_MemoryLeak_Prevention(t *testing.T) {
	...
	for i := range numKeys {
		...
		result := limiter.waitWithCancellation(i, executionTime, done)
	...
	for i := range concurrency {
		go func(i int) {
			...
			for k := range keysPerGoroutine {
				...
				// Then try to wait (which will timeout immediately)
				limiter.waitWithCancellation(keyID, executionTime, done)
			}
		}(i)
	}
	wg.Wait()
...
func TestLimiter_Waiters_Cleanup_WithSuccessfulWaits(t *testing.T) {
	...
	for i := range concurrency {
		go func(i int) {
			...
			results[i] = limiter.waitWithCancellation("key", executionTime, done)
		}(i)
	}
	wg.Wait()
